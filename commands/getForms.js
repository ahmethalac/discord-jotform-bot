module.exports = {
  name: 'getForms',
  execute(message, args, jotform, Discord, prefix) {
    jotform.getForms()
      .then((value) => {
        const formArray = [];
        ...
        message.channel.send(replyMessage)
          .then(() => {
            ...
            collector.on('collect', (collected) => {
              const formNumber = collected.content.slice(prefix.length).trim().split(/ +/);
              formNumber.shift();
              if (formNumber.length !== 1) {
                message.channel.send('Wrong command! Try again "!submissions formNumber" command');
                ...
                jotform.getFormSubmissions(formArray[Number(formNumber[0]) - 1].id)
                  .then((r) => {
                    // Burayı daha düzenleyemedim ama submissionları alıyor
